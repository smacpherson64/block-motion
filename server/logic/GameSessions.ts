...
import { createMachine } from "https://unpkg.com/xstate@4/dist/xstate.js";
import { useMachine } from "https://unpkg.com/@xstate/react/dist/xstate-react.umd.min.js";
...
export const useGameSession = (options: { game: string }) => {
  const [state, send] = useMachine(GameSessionMachine);

  React.useEffect(() => {
    getInitialSession({ game: options.game }).then(send);
  }, [options.game, send]);

  return React.useMemo(() => [state, send] as const, [state, send]);
};
